...
export default function SecondStep() {
  ...
  const onSubmit = (data) => {
    ...
    axios
      .post(
        `https://prod-in-api.hiringplug.com/manage-web-service/v1/create-demo`,
        postData
      )
      .then((res) => {
        setIsLoading(false)
  ...
  return (
    ...
    >
      ...
      <label className="w-full flex items-start justify-center flex-col">
        <p className="text-[15px] font-semibold text-gray-800">
          <span title="Please share some information about yourself and your specific queries, if any to help us prepare better for the interaction" className="text-red-800"></span> Your Query :
        </p>
        <textarea
          name="enquiry"
          {...register("enquiry")}
      ...
      <button
        type="submit"
        className={`${isLoading ? "bg-green-400" : "bg-[#00a63f]"} w-full  h-10 text-white text-[15px] rounded-[4px]`}
      >
